...
public class ChaseIdMovedAfterApprove_TC19 extends TestBase {
    LoginOut objLoginOut = new LoginOut();
    ThirdParty objThirdParty = new ThirdParty();
    ApprovalCenter objApprovalCenter=new ApprovalCenter();

    @Test(description = "Verify that chase can be moved from one AID to another AID after approval .", groups = {"Third party"})
    public void ChaseIdMovedAfterApprove_TC19() throws Exception {
        ...
        String DestinationAID=$x("//tr[2]//td[2]").getText();
        ...
        sleep(2000);
        $x("//th[contains(text(),'Status')]//following::span[contains(text(),'Approve')]").click();
        sleep(2000);
        ...
        $x("//span[@class='ui-tabview-title ng-star-inserted'][contains(text(),'AID')]").click();
        sleep(2000);
        $x("//label[contains(text(),'AID')]//following::input").setValue(DestinationAID);
        sleep(2000);
        $(ProjectsRepo.Update).click();
        sleep(5000);
        $x("//tr[1]//td[1]").click();
        ...
        for(int i=0;i<=size-1;i++){
            ...
            if(ActualChaseId.equals(OldChaseId)){
                logTestStepPass("Chase Id successfully moved to new AID");
            }else{
