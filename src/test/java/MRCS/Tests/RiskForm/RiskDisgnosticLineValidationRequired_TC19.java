...
public class RiskDisgnosticLineValidationRequired_TC19 extends TestBase {
    LoginOut objLoginOut = new LoginOut();
    WaitTool objWait = new WaitTool();
    Risk objRisk = new Risk();

    @Test(description = "Validation message should be displayed for diagnostic line validations.", groups = {"RiskForm"})
    public void RiskDisgnosticLineValidationRequired_TC19() throws Exception {
        ...
        $x("(//div[@class='button button__add'])[2]").click();
        sleep(2000);
        objRisk.ChecklistForART();
        sleep(3000);
        $x("(//span[contains(text(),'Submit')])[2]").click();
        sleep(5000);
        if(WebDriverRunner.getWebDriver().getPageSource().contains("Diagnosis missing PageNumber or ValidationReasonCode")){
