...
public class RiskNoteRequiredForServiceStartDate_TC8 extends TestBase {
    LoginOut objLoginOut = new LoginOut();
    WaitTool objWait = new WaitTool();
    Risk objRisk = new Risk();

    @Test(description = "Validation message should be displayed if note is not entered in service start date field", groups = {"RiskForm"})
    public void RiskNoteRequiredForServiceStartDate_TC8() throws Exception {
        ...
        if ($x("(//h3[contains(text(),'Diagnoses')]//preceding::span[@class='ui-paginator-pages'])").isDisplayed()) {
            while ($x("(//h3[contains(text(),'Diagnoses')]//preceding::a[@class='ui-paginator-next ui-paginator-element ui-state-default ui-corner-all'])").isDisplayed() || $x("//tr[1]//td[1]").exists()) {
                ...
                for (int i = 1; i <= page ; i++) {
                    ...
                    if (result == 1 && i == 1) {
                        Log.info("in if loop");
                        objRisk.NoteRequiredForServiceStartDate();
                        sleep(2000);
        ...
        sleep(5000);
        if(WebDriverRunner.getWebDriver().getPageSource().contains("EncounterServiceDateFrom Note must be provided")){
