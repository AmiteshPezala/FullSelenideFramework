...
public class RiskAddedEncounterMustHaveDiagnosis_TC21 extends TestBase {
    LoginOut objLoginOut = new LoginOut();
    WaitTool objWait = new WaitTool();
    Risk objRisk = new Risk();

    @Test(description = "Validation message should be displayed that encounter must have diagnosis.", groups = {"RiskForm"})
    public void RiskAddedEncounterMustHaveDiagnosis_TC21() throws Exception {
        ...
        objRisk.AddNewEncounterButNotDiagnosis();
        sleep(2000);
        objRisk.ChecklistForART();
        $x("(//span[contains(text(),'Submit')])[2]").click();
        sleep(5000);
        if(WebDriverRunner.getWebDriver().getPageSource().contains("Encounter must have at least one diagnosis")){
            logTestStepPass("Validation message displayed");
        }else{
            logTestStepFail("Validation message not displayed");
        }
        sleep(2000);
        objRisk.DeleteEncounter();
        objLoginOut.logout();
    }
}
