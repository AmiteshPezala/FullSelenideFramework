...
public class BulkUpdate_VerifyPendClose_TC10 extends TestBase {
    LoginOut objLoginOut = new LoginOut();
    WaitTool objWait = new WaitTool();
    BulkChanges objBulkAction = new BulkChanges();

    @Test(description = "Verify pend close functionality.", groups = {"Bulk Update"})
    public void BulkUpdate_VerifyPendClose_TC10() throws Exception {
        objLoginOut.loginAs(LoginOut.Actor.USERNAME_USERS);
        logTestStep("Log in to application");
        sleep(2000);
        objBulkAction.GettingPendIdToClose();
        sleep(2000);
        if ($x("//tr[1]//td[2]").isDisplayed() && $x("//tr[2]//td[2]").isDisplayed()) {
            ...
            if (Status1.equals("Success") && Status2.equals("Success")) {
                ...
                sleep(2000);
                if(CurrentStatus1.equals("Closed") && CurrentStatus2.equals("Closed")){
                    logTestStepPass("Both RetrievalPend Id status changed to closed state.");
                }else{
                    logTestStepFail("RetrievalPend Id status not changed to closed.");
                }
            }
            else{
