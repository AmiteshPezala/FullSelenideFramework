...
public class HST_VerifyPageNoRequiredForServiceEndDate_TC45 extends TestBase {
    LoginOut objLoginOut = new LoginOut();
    Risk objRisk = new Risk();

    @Test(description = "Verify that page no of service end  date is a required field.", groups = {"IVA HST"})
    public void HST_VerifyPageNoRequiredForServiceEndDate_TC45() throws Exception {
        ...
        if ($x("(//h3[contains(text(),'Diagnoses')]//preceding::span[@class='ui-paginator-pages'])").isDisplayed()) {
            while ($x("(//h3[contains(text(),'Diagnoses')]//preceding::a[@class='ui-paginator-next ui-paginator-element ui-state-default ui-corner-all'])").isDisplayed() || $x("//tr[1]//td[1]").exists()) {
                ...
                for (int i = 1; i <= page ; i++) {
                    ...
                    if (result == 1 && i == 1) {
                        Log.info("in if loop");
                        objRisk.PageNoIsRequiredForServiceEndDate();
                        sleep(2000);
            ...
            objRisk.ServiceEndDateIsRequired();
            sleep(2000);
