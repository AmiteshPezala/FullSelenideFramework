...
public class HST_PageNoRequiredForSignatureLastName_TC54 extends TestBase {
    LoginOut objLoginOut = new LoginOut();
    Risk objRisk = new Risk();

    @Test(description = "Verify that message is displayed if page no not present for signature last name.", groups = {"IVA HST"})
    public void HST_PageNoRequiredForSignatureLastName_TC54() throws Exception {
        ...
        if ($x("(//h3[contains(text(),'Diagnoses')]//preceding::span[@class='ui-paginator-pages'])").isDisplayed()) {
            while ($x("(//h3[contains(text(),'Diagnoses')]//preceding::a[@class='ui-paginator-next ui-paginator-element ui-state-default ui-corner-all'])").isDisplayed() || $x("//tr[1]//td[1]").exists()) {
                ...
                for (int i = 1; i <= page ; i++) {
                    ...
                    if (result == 1 && i == 1) {
                        Log.info("in if loop");
                        objRisk.PageNoOfSignatureLastName();
                        sleep(2000);
                    } else {
        ...
        sleep(5000);
        if(WebDriverRunner.getWebDriver().getPageSource().contains("Encounter Acceptable Signature LastName, PageNumber must be provided")){
