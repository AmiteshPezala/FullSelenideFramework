...
public class HST_VerifyNoteRequiredForProvider_TC52 extends TestBase {
    LoginOut objLoginOut = new LoginOut();
    Risk objRisk = new Risk();

    @Test(description = "Verify that message is displayed if note not entered in provider field.", groups = {"IVA HST"})
    public void HST_VerifyNoteRequiredForProvider_TC52() throws Exception {
        ...
        if ($x("(//h3[contains(text(),'Diagnoses')]//preceding::span[@class='ui-paginator-pages'])").isDisplayed()) {
            while ($x("(//h3[contains(text(),'Diagnoses')]//preceding::a[@class='ui-paginator-next ui-paginator-element ui-state-default ui-corner-all'])").isDisplayed() || $x("//tr[1]//td[1]").exists()) {
                ...
                for (int i = 1; i <= page ; i++) {
                    ...
                    if (result == 1 && i == 1) {
                        Log.info("in if loop");
                        objRisk.NoteIsRequiredForProviderTransformation();
                        sleep(2000);
                    } else {
        ...
        sleep(5000);
        if(WebDriverRunner.getWebDriver().getPageSource().contains("EncounterProvider Note must be provided")){
