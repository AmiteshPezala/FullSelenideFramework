...
public class HST_VerifyServiceDateFromValidation_TC62 extends TestBase {
    LoginOut objLoginOut = new LoginOut();
    Risk objRisk = new Risk();

    @Test(description = "Validation message should be displayed if service from date is greater than service end date.", groups = {"IVA HST"})
    public void HST_VerifyServiceDateFromValidation_TC62() throws Exception {
        ...
        if ($x("(//h3[contains(text(),'Diagnoses')]//preceding::span[@class='ui-paginator-pages'])").isDisplayed()) {
            while ($x("(//h3[contains(text(),'Diagnoses')]//preceding::a[@class='ui-paginator-next ui-paginator-element ui-state-default ui-corner-all'])").isDisplayed() || $x("//tr[1]//td[1]").exists()) {
                ...
                for (int i = 1; i <= page ; i++) {
                    ...
                    if (result == 1 && i == 1) {
                        Log.info("in if loop");
                        objRisk.EncountervalidationFormDateLessThanEndDate();
                        sleep(2000);
                    } else {
        ...
        sleep(5000);
        if(WebDriverRunner.getWebDriver().getPageSource().contains("Encounter service date through should be later than service date from")){
