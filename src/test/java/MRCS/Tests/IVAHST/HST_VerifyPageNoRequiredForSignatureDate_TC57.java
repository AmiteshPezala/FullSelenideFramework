...
public class HST_VerifyPageNoRequiredForSignatureDate_TC57 extends TestBase {
    LoginOut objLoginOut = new LoginOut();
    Risk objRisk = new Risk();

    @Test(description = "Validation message should be displayed page no is required.", groups = {"IVA HST"})
    public void HST_VerifyPageNoRequiredForSignatureDate_TC57() throws Exception {
        ...
        if ($x("(//h3[contains(text(),'Diagnoses')]//preceding::span[@class='ui-paginator-pages'])").isDisplayed()) {
            while ($x("(//h3[contains(text(),'Diagnoses')]//preceding::a[@class='ui-paginator-next ui-paginator-element ui-state-default ui-corner-all'])").isDisplayed() || $x("//tr[1]//td[1]").exists()) {
                ...
                for (int i = 1; i <= page ; i++) {
                    ...
                    if (result == 1 && i == 1) {
                        Log.info("in if loop");
                        objRisk.PageNoOfSignatureField();
                        sleep(2000);
                    } else {
