...
public class HST_VerifyMembersDOBField_TC42 extends TestBase {
    LoginOut objLoginOut = new LoginOut();
    Risk objRisk = new Risk();

    @Test(description = "Verify that member's date of birth in document field is a required field.", groups = {"IVA HST"})
    public void HST_VerifyMembersDOBField_TC42() throws Exception {
        ...
        if ($x("(//h3[contains(text(),'Diagnoses')]//preceding::span[@class='ui-paginator-pages'])").isDisplayed()) {
            while ($x("(//h3[contains(text(),'Diagnoses')]//preceding::a[@class='ui-paginator-next ui-paginator-element ui-state-default ui-corner-all'])").isDisplayed() || $x("//tr[1]//td[1]").exists()) {
                ...
                for (int i = 1; i <= page; i++) {
                    ...
                    if (result == 1 && i == 1) {
                        Log.info("in if loop");
                        objRisk.membersDateOfBirthInDocument();
                        sleep(2000);
                    } else {
        ...
        {
            logTestStepPass("Validation message displayed to fill member's date of birth in document field");
        }else {
            logTestStepFail("Validation message not displayed to fill member's date of birth in document field");
        }
        sleep(2000);
        objLoginOut.logout();
    }
