...
public class COL_VerifyColateralCancerScreeningShowsPositiveCompliance_TC2 extends TestBase {
    LoginOut objLoginOut = new LoginOut();
    Risk objRisk= new Risk();

    @Test( description = "Verify colateral cancer screening shows +ve compliance when  colonoscopy in 2019 or 9 Years Prior  and result = yes/medical hx", groups = {"Hedis COL"} )
...
//        System.out.println(CurrentDate);
        $(HedisCOLRepo.ColonoscopyDate).setValue("12/31/2018");
        sleep(2000);
        $(HedisCOLRepo.ColonoscopyPageNumber).setValue("1");
        sleep(2000);
        $x("(//span[@class='ui-button-icon-left ui-clickable pi pi-caret-down'])[1]").click();
        sleep(2000);
